...
def download_events_from_sa_to_pickle(project='tvcbook', remark='default', start_day='2018-11-27', end_day='2019-09-25'):
    ...
    for day in date_list:
        for hour in range(0, 24):
            ...
            sql = """select * from events where date='{date}' and time >= '{time_start}' and time< '{time_end}'""".format(date=day, time_start=time_start,
                                                                                                                          time_end=time_end)
    ...
    for pkl in filelist:
        ...
        for item in results:
            ...
            try:
                ...
                insert_data(project='tvcbook', data_decode=all_json, User_Agent=None, Host=None, Connection=None, Pragma=None, Cache_Control=None, Accept=None,
                            Accept_Encoding=None, Accept_Language=None, ip=itemdict["$ip"] if "$ip" in itemdict else None, ip_city=ip_city, ip_asn=ip_asn,
                            url=None, referrer=itemdict["$referrer"] if "$referrer" in itemdict else None, remark=remark,
                            ua_platform=itemdict["$lib"] if "$lib" in itemdict else None, ua_browser=itemdict["$browser"] if "$browser" in itemdict else None,
                            ua_version=itemdict["$browser_version"] if "$browser_version" in itemdict else None, ua_language=None, ip_is_good=ip_is_good,
                            ip_asn_is_good=ip_asn_is_good, created_at=created_at)
            except Exception:
...
def import_shortcut_from_sa_to_sql(project='your_project_name', domain='your_domain'):
    ...
    for i in result:
        ...
        insert_shortcut(project=project, short_url=short_url_key, long_url=short_url_target, expired_at=2147443200, src='sensors',
                        src_short_url=domain + short_url_key, submitter='importer', utm_source=utm_source, utm_medium=utm_medium, utm_campaign=utm_campaign,
