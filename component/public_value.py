    ...
    if timer:
        ...
        if tday and tmon and tweek:
            for thour in thours:
                for tmin in tmins:
                    times.append(
                        {'time_tuple': time.strptime('{yy}-{mm}-{dd} {hh}:{mi}:00'.format(yy=cyear, mm=cmon, dd=cday, hh=thour, mi=tmin), '%Y-%m-%d %H:%M:%S'),
                         'time_int': int(time.mktime(
                             time.strptime('{yy}-{mm}-{dd} {hh}:{mi}:00'.format(yy=cyear, mm=cmon, dd=cday, hh=thour, mi=tmin), '%Y-%m-%d %H:%M:%S')))})
    else:
        times = [{'time_tuple': time.strptime('{yy}-{mm}-{dd} {hh}:{mi}:00'.format(yy=cyear, mm=cmon, dd=cday, hh=chour, mi=cmin), '%Y-%m-%d %H:%M:%S'),
                  'time_int': int(
                      time.mktime(time.strptime('{yy}-{mm}-{dd} {hh}:{mi}:00'.format(yy=cyear, mm=cmon, dd=cday, hh=chour, mi=cmin), '%Y-%m-%d %H:%M:%S')))}]
    return times
...
def get_time_array_from_nlp(time_nlp):
    ...
    if len(time_str) == 5:
        ...
        time_str = '00:00:00'
    times = {'time_tuple': time.strptime(date + ' ' + time_str, '%Y-%m-%d %H:%M:%S'),
             'time_int': int(time.mktime(time.strptime(date + ' ' + time_str, '%Y-%m-%d %H:%M:%S'))), 'date_str': date, 'hour_str': time_str}
    return times
