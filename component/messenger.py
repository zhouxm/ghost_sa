...
class send:
    ...
    def via_email(self):
        default_mail_from = self.noti_content['mail_from'] if 'mail_from' in self.noti_content and self.noti_content['mail_from'] and self.noti_content[
            'mail_from'] != '' else 'accounts-noreply@notify.tvcbook.com'  # 默认发送邮箱
        result = send_email(to_addr=self.noti_content['mail_to'], from_addr=default_mail_from, subject=self.noti_content['subject'],
                            html=self.noti_content['content'])
        return result
...
def send_manual(project, noti):
    ...
    if send_result:
        ...
        # print(send_tracker)
        insert_event(table=project, alljson=json.dumps(send_tracker), track_id=0, distinct_id=noti[4], lib='noti', event='recall', type_1='track',
                     User_Agent='', Host='', Connection='', Pragma='', Cache_Control='', Accept='', Accept_Encoding='', Accept_Language='', ip='', ip_city='{}',
