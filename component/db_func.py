...
def insert_event(table, alljson, track_id, distinct_id, lib, event, type_1, User_Agent, Host, Connection, Pragma, Cache_Control, Accept, Accept_Encoding,
                 Accept_Language, ip, ip_city, ip_asn, url, referrer, ua_platform, ua_browser, ua_version, ua_language, remark='normal', created_at=None):
...
def insert_devicedb(table, distinct_id, device_id, manufacturer, model, os, os_version, screen_width, screen_height, network_type, user_agent, accept_language,
                    ip, ip_city, ip_asn, wifi, app_version, carrier, referrer, referrer_host, bot_name, browser, browser_version, is_login_id,
                    screen_orientation, gps_latitude, gps_longitude, first_visit_time, first_referrer, first_referrer_host, first_browser_language,
                    first_browser_charset, first_search_keyword, first_traffic_source_type, utm_content, utm_campaign, utm_medium, utm_term, utm_source,
                    latest_utm_content, latest_utm_campaign, latest_utm_medium, latest_utm_term, latest_utm_source, latest_referrer, latest_referrer_host,
                    latest_search_keyword, latest_traffic_source_type, update_content, ua_platform, ua_browser, ua_version, ua_language, lib, created_at=None,
                    updated_at=None):
    if created_at is None:
        timenow = int(time.time())
        date = time.strftime("%Y-%m-%d", time.localtime())
...
def insert_user_db(project, distinct_id, lib, map_id, original_id, user_id, all_user_profile, update_params, created_at=None, updated_at=None):
    ...
    sql = """set @@tidb_disable_txn_auto_retry = 0;
set @@tidb_retry_limit = 10;
    insert HIGH_PRIORITY into `{table}_user` (`distinct_id`,`lib`,`map_id`,`original_id`,`user_id`,`all_user_profile`,`created_at`,`updated_at`) values (%(distinct_id)s,%(lib)s,%(map_id)s,%(original_id)s,%(user_id)s,%(all_user_profile)s,%(created_at)s,%(updated_at)s) ON DUPLICATE KEY UPDATE `updated_at`={updated_at}{update_params}""".format(
        table=project, update_params=update_params, updated_at=timenow)
    key = {'distinct_id': distinct_id, 'lib': lib, 'map_id': map_id, 'original_id': original_id, 'user_id': user_id, 'all_user_profile': all_user_profile,
...
set @@tidb_retry_limit = 10;
    insert HIGH_PRIORITY into `{table}_properties` (`lib`,`remark`,`event`,`properties`,`properties_len`,`created_at`,`updated_at`,`total_count`,`lastinsert_at`) values ( %(lib)s,%(remark)s,%(event)s,%(properties)s,%(properties_len)s,%(created_at)s,%(updated_at)s,1,%(updated_at)s) ON DUPLICATE KEY UPDATE `properties`=if(properties_len<%(properties_len)s,%(properties)s,properties),`properties_len`=if(properties_len<%(properties_len)s,%(properties_len)s,properties_len),updated_at=if(properties_len<%(properties_len)s,%(updated_at)s,updated_at),total_count=total_count+1,lastinsert_at=%(updated_at)s;""".format(
        table=project)
    key = {'lib': lib, 'remark': remark, 'event': event, 'properties': properties, 'properties_len': properties_len, 'created_at': created_at,
...
def check_long_url(long_url):
    ...
    if result[1] > 0:
        ...
        for item in result[0]:
            ...
            long_url = item[12]
            result_dict.append(
                {'project': project, 'short_url': short_url, 'expired_at': expired_at, 'created_at': created_at, 'src': src, 'src_short_url': src_short_url,
                 'submitter': submitter, 'utm_source': utm_source, 'utm_medium': utm_medium, 'utm_campaign': utm_campaign, 'utm_content': utm_content,
                 'utm_term': utm_term, 'long_url': long_url})
...
def insert_shortcut(project, short_url, long_url, expired_at, src, src_short_url, submitter, utm_source, utm_medium, utm_campaign, utm_content, utm_term):
    ...
    sql = """insert into shortcut (`project`,`short_url`,`long_url`,`expired_at`,`created_at`,`src`,`src_short_url`,`submitter`,`utm_source`,`utm_medium`,`utm_campaign`,`utm_content`,`utm_term`) values ('{project}','{short_url}','{long_url}',{expired_at},{created_at},'{src}','{src_short_url}','{submitter}','{utm_source}','{utm_medium}','{utm_campaign}','{utm_content}','{utm_term}')""".format(
        project=project, short_url=short_url, long_url=long_url, expired_at=expired_at, created_at=timenow, src=src, src_short_url=src_short_url,
        submitter=submitter, utm_source=utm_source, utm_medium=utm_medium, utm_campaign=utm_campaign, utm_content=utm_content, utm_term=utm_term).replace(
...
def insert_shortcut_history(short_url, result, cost_time, ip, user_agent, accept_language, ua_platform, ua_browser, ua_version, ua_language, created_at=None):
    ...
    sql = """insert HIGH_PRIORITY shortcut_history (`short_url`,`result`,`cost_time`,`ip`,`created_at`,`user_agent`,`accept_language`,`ua_platform`,`ua_browser`,`ua_version`,`ua_language`) values ('{short_url}','{result}',{cost_time},'{ip}',{created_at},'{user_agent}','{accept_language}','{ua_platform}','{ua_browser}','{ua_version}','{ua_language}')""".format(
        short_url=short_url, result=result, cost_time=cost_time, ip=ip, created_at=created_at if created_at else timenow, user_agent=user_agent,
        accept_language=accept_language, ua_platform=ua_platform, ua_browser=ua_browser, ua_version=ua_version, ua_language=ua_language).replace("'None'",
                                                                                                                                                 "Null").replace(
        "None", "Null")
    result = do_tidb_exe(sql)
...
def show_check(project, date, hour, order, start, limit, add_on_where):
    sql = """select distinct_id,event,type,all_json,host,user_agent,ip,url,remark,from_unixtime(created_at) from {project} where date = '{date}' and hour = {hour} {add_on_where} order by created_at {order} limit {start},{limit}""".format(
        project=project, date=date, hour=hour, order=order, start=start, limit=limit, add_on_where=add_on_where)
    result = do_tidb_select(sql)
    if result[1] == 0:
...
def insert_mobile_ad_src(src, src_name, src_args, utm_source, utm_medium, utm_campaign, utm_content, utm_term):
    timenow = int(time.time())
    sql = """insert mobile_ad_src (`src`,`src_name`,`src_args`,`created_at`,`updated_at`,`utm_source`,`utm_medium`,`utm_campaign`,`utm_content`,`utm_term`) values ('{src}','{src_name}','{src_args}',{created_at},{updated_at},'{utm_source}','{utm_medium}','{utm_campaign}','{utm_content}','{utm_term}') ON DUPLICATE KEY UPDATE `src_name`='{src_name}',`src_args`='{src_args}',`updated_at`={updated_at},`utm_source`='{utm_source}',`utm_medium`='{utm_medium}',`utm_campaign`='{utm_campaign}',`utm_content`='{utm_content}',`utm_term`='{utm_term}' """.format(
        src=src, src_name=src_name, src_args=src_args, created_at=timenow, updated_at=timenow, utm_source=utm_source, utm_medium=utm_medium,
        utm_campaign=utm_campaign, utm_content=utm_content, utm_term=utm_term).replace("'None'", "Null").replace("None", "Null")
    result = do_tidb_exe(sql)
    if result[1] == 0:
...
def insert_mobile_ad_list(project, url, src, src_url, submitter, utm_source, utm_medium, utm_campaign, utm_content, utm_term, expired_at=2147483647):
    ...
    sql = """insert mobile_ad_list (`project`,`url`,`expired_at`,`created_at`,`src`,`src_url`,`submitter`,`utm_source`,`utm_medium`,`utm_campaign`,`utm_content`,`utm_term`) values ('{project}','{url}',{expired_at},{created_at},'{src}','{src_url}','{submitter}','{utm_source}','{utm_medium}','{utm_campaign}','{utm_content}','{utm_term}')""".format(
        project=project, url=url, expired_at=expired_at, created_at=timenow, src=src, src_url=src_url, submitter=submitter, utm_source=utm_source,
        utm_medium=utm_medium, utm_campaign=utm_campaign, utm_content=utm_content, utm_term=utm_term).replace("'None'", "Null").replace("None", "Null")
    result = do_tidb_exe(sql)
    if result[1] == 0:
...
def read_mobile_ad_list(page, length, filters='', sort='created_at', way='desc'):
    ...
    from mobile_ad_list left join mobile_ad_src on mobile_ad_list.src=mobile_ad_src.src {filters} ORDER BY {sort} {way} Limit {start_pageline},{length}""".format(
        start_pageline=(page - 1) * length, length=length, filters=filters, sort=sort, way=way)
    print(sql)
    result = do_tidb_select(sql)
    if result[1] == 0:
...
def read_mobile_ad_src_list(add_on_where=''):
    timenow = int(time.time())
    sql = """select src,src_name,src_args,from_unixtime(created_at),from_unixtime(updated_at),if({timenow}-created_at<=604800,1,0),utm_source,utm_medium,utm_campaign,utm_content,utm_term from mobile_ad_src {add_on_where} order by src_name""".format(
        add_on_where=add_on_where, timenow=timenow)
    result = do_tidb_select(sql)
    if result[1] == 0:
...
def check_mobile_ad_url(url):
    ...
    if result[1] > 0:
        ...
        for item in result[0]:
            result_dict.append(
                {'project': item[0], 'url': item[1], 'expired_at': item[2], 'created_at': item[3], 'src': item[4], 'src_url': item[5], 'submitter': item[6],
                 'utm_source': item[7], 'utm_medium': item[8], 'utm_campaign': item[9], 'utm_content': item[10], 'utm_term': item[11]})
        return result_dict, 'exist'
    else:
        return '', 'empty'
...
def find_recall_url(project, device_id, created_at):
    ...
    sql = """select JSON_EXTRACT(all_json,'$."properties"."callback_url"'),all_json,distinct_id from {project} where date >= DATE_SUB('{date}',INTERVAL {day_diff} day) and date <= '{date}' and `event` = '$AppChannelMatching' and distinct_id in ('{device_id}',md5('{device_id}'),replace('{device_id}','"',''),md5(replace('{device_id}','"',''))) order by created_at desc limit 1""".format(
        project=project, device_id=device_id, date=date, day_diff=admin.aso_dsp_callback_interval_days)
    result = do_tidb_select(sql)
    return result[0], result[1]
...
def insert_shortcut_read(short_url, ip, user_agent, accept_language, ua_platform, ua_browser, ua_version, ua_language, referrer, created_at=None):
    timenow = int(time.time())
    sql = """insert HIGH_PRIORITY shortcut_read (`short_url`,`ip`,`created_at`,`user_agent`,`accept_language`,`ua_platform`,`ua_browser`,`ua_version`,`ua_language`,`referrer`) values ('{short_url}','{ip}',{created_at},'{user_agent}','{accept_language}','{ua_platform}','{ua_browser}','{ua_version}','{ua_language}','{referrer}')""".format(
        short_url=short_url, ip=ip, created_at=created_at if created_at else timenow, user_agent=user_agent, accept_language=accept_language,
        ua_platform=ua_platform, ua_browser=ua_browser, ua_version=ua_version, ua_language=ua_language, referrer=referrer).replace("'None'", "Null").replace(
        "None", "Null")
    result = do_tidb_exe(sql)
    print('已插入解析记录' + str(result[1]))
...
def insert_usergroup_data(project, group_list_id, data_index, key, json, enable, created_at=None, updated_at=None):
    timenow = int(time.time())
    sql = """insert {project}_usergroup_data (`group_list_id`,`data_index`,`data_key`,`data_json`,`enable`,`created_at`,`updated_at`) values (%(group_list_id)s,%(data_index)s,%(data_key)s,%(data_json)s,%(enable11)s,%(created_at)s,%(updated_at)s)""".format(
        project=project)
    key = {'group_list_id': group_list_id, 'data_index': data_index, 'data_key': key, 'data_json': json, 'enable11': enable,
           'created_at': created_at if created_at else timenow, 'updated_at': updated_at if updated_at else timenow}
    result = do_tidb_exe(sql=sql, args=key)
    return result[1]


def insert_usergroup_list(project, group_id, group_index, list_desc=None, list_init_date=None, status=0, complete_at=0, apply_temple_times=0, item_add=0,
                          created_at=None, updated_at=None, jobs_id=0):
    timenow = int(time.time())
    sql = """insert {project}_usergroup_list (`group_id`,`group_list_index`,`list_init_date`,`list_desc`,`item_count`,`status`,`complete_at`,`apply_temple_times`,`created_at`,`updated_at`,`jobs_id`) values ({group_id},{group_index},{list_init_date},'{list_desc}',{item_add},{status},{complete_at},{apply_temple_times},{created_at},{updated_at},{jobs_id}) ON DUPLICATE KEY UPDATE `complete_at`=if({complete_at}=0,`complete_at`,{complete_at}),`apply_temple_times`=`apply_temple_times`+{apply_temple_times},`status`=if ({status}=0,`status`,{status}),`item_count`=if({item_add}=0,`item_count`,`item_count`+{item_add}),updated_at={updated_at}""".format(
        project=project, jobs_id=jobs_id, group_id=group_id, group_index=group_index, list_init_date=list_init_date if list_init_date else timenow,
        list_desc=list_desc, status=status, complete_at=complete_at, apply_temple_times=apply_temple_times, item_add=item_add,
        created_at=created_at if created_at else timenow, updated_at=updated_at if updated_at else timenow)
    result = do_tidb_exe(sql=sql)
    return result[0]


def update_usergroup_plan(project, plan_id, latest_data_list_index=0, latest_apply_temple_time=0, latest_apply_temple_id=0, updated_at=None, repeat_times_add=0,
                          latest_data_time=0):
    timenow = int(time.time())
    ...
    where id ={plan_id}""".format(project=project, plan_id=plan_id, latest_data_list_index=latest_data_list_index,
                                  latest_apply_temple_time=latest_apply_temple_time, latest_apply_temple_id=latest_apply_temple_id,
                                  updated_at=updated_at if updated_at else timenow, repeat_times_add=repeat_times_add, latest_data_time=latest_data_time)
    result = do_tidb_exe(sql=sql)
    return result[0], result[1]


def update_usergroup_list(project, list_id, apply_temple_times, updated_at=None):
    sql = """update {project}_usergroup_list set apply_temple_times=apply_temple_times+{apply_temple_times},updated_at={updated_at} where id={list_id}""".format(
        apply_temple_times=int(apply_temple_times) if int(apply_temple_times) else 0, project=project,
        updated_at=updated_at if updated_at else int(time.time()), list_id=list_id)
    result = do_tidb_exe(sql=sql)
    return result[0], result[1]
...
def check_list_id(project, group_id, group_list_index):
    sql = """select id,item_count,status from {project}_usergroup_list where group_id={group_id} and group_list_index={group_list_index}""".format(
        project=project, group_id=group_id, group_list_index=group_list_index)
    result = do_tidb_exe(sql=sql)
    return result[0]
...
def check_next_scheduler_job(priority=13, current_time=None):
    ...
    if priority == 13:
        ...
        priorities = '14,15'
    sql = """select id,project,group_id,datetime,data from scheduler_jobs where priority={priority} and datetime<={current_time}  and `status` = 16 order by id limit 1""".format(
        priority=priority, current_time=current_time)
    result = do_tidb_exe(sql=sql)
    return result[0], result[1]


def update_scheduler_job(jobid, list_index=0, status=0):
    sql = """update scheduler_jobs set list_index = if({list_index}!=0,{list_index},list_index),status=if({status}!=0,{status},status),updated_at={updated_at} where id = {jobid}""".format(
        jobid=jobid, list_index=list_index, status=status, updated_at=int(time.time()))
    result = do_tidb_exe(sql=sql)
    return result[0], result[1]
...
def insert_scheduler_job(project, group_id, datetime, data={}, priority=13, status=11):
    import json
    sql = """insert ignore scheduler_jobs (`project`,`group_id`,`datetime`,`data`,`priority`,`status`,`created_at`,`updated_at`) values ('{project}',{group_id},{datetime},'{data}',{priority},{status},{created_at},{updated_at})""".format(
        project=project, group_id=group_id, datetime=datetime, data=json.dumps(data), priority=priority, status=status, created_at=int(time.time()),
        updated_at=int(time.time()))
    result = do_tidb_exe(sql=sql)
    return result[0], result[1]
...
def insert_noti(project, type_1, created_at, updated_at, distinct_id, content, send_at, plan_id=0, list_id=0, data_id=0, temple_id=0, noti_group_id=0,
                priority=13, status=9, owner='noti', recall_result=None):
    import json
    sql = """insert ignore {project}_noti (`distinct_id`,`plan_id`,`list_id`,`data_id`,`temple_id`,`noti_group_id`,`priority`,`status`,`owner`,`type`,`content`,`send_at`,`recall_result`,`created_at`,`updated_at`) values (%(distinct_id)s,%(plan_id)s,%(list_id)s,%(data_id)s,%(temple_id)s,%(noti_group_id)s,%(priority)s,%(status)s,%(owner)s,%(type)s,%(content)s,%(send_at)s,%(recall_result)s,%(created_at)s,%(updated_at)s)""".format(
        project=project)
    key = {'distinct_id': distinct_id, 'type': type_1, 'plan_id': plan_id, 'list_id': list_id, 'data_id': data_id, 'temple_id': temple_id,
           'noti_group_id': noti_group_id, 'priority': priority, 'status': status, 'owner': owner, 'content': json.dumps(content),
           'recall_result': recall_result, 'send_at': send_at if send_at else int(time.time()), 'created_at': created_at if created_at else int(time.time()),
           'updated_at': updated_at if updated_at else int(time.time())}
    result = do_tidb_exe(sql=sql, args=key)
    return result[0], result[1]


def update_noti(project, noti_id, updated_at, status=0, recall_result=None):
    sql = """update {project}_noti set status = if({status}=0,status,{status}) , recall_result = %(recall_result)s , updated_at = {updated_at} where id = {noti_id}""".format(
        project=project, noti_id=noti_id, updated_at=updated_at if updated_at else int(time.time()), status=status, recall_result=recall_result)
...
def select_noti_auto(project, priority=13, status=8):
    sql = """select id,noti_group_id,type,content,distinct_id from {project}_noti where status={status} and priority={priority} and send_at<={send_at} limit 1""".format(
        project=project, status=status, priority=priority, send_at=int(time.time()))
    result = do_tidb_select(sql=sql)
    return result[0], result[1]


def select_noti_group(project, noti_group_id, status=9):
    sql = """select id,noti_group_id,type,content,distinct_id from {project}_noti where noti_group_id = {noti_group_id} and status = {status}""".format(
        project=project, noti_group_id=noti_group_id, status=status, send_at=int(time.time()))
    result = do_tidb_select(sql=sql)
    return result[0], result[1]


def select_noti_single(project, noti_id, status=9):
    sql = """select id,noti_group_id,type,content,distinct_id from {project}_noti where id = {noti_id} and status = {status}""".format(project=project,
                                                                                                                                       noti_id=noti_id,
                                                                                                                                       status=status,
...
def insert_noti_group(project, plan_id, list_id, data_id, temple_id, owner=None, send_at=0, sent=0, total=0, priority=13, status=8, created_at=None,
                      updated_at=None):
    created_at = created_at if created_at else int(time.time())
    updated_at = created_at if created_at else int(time.time())
    sql = """insert {project}_noti_group (plan_id,list_id,data_id,temple_id,priority,status,owner,send_at,sent,total,created_at,updated_at ) values (%(plan_id)s,%(list_id)s,%(data_id)s,%(temple_id)s,%(priority)s,%(status)s,%(owner)s,%(send_at)s,%(sent)s,%(total)s,%(created_at)s,%(updated_at)s)""".format(
        project=project)
    key = {'plan_id': plan_id, 'list_id': list_id, 'data_id': data_id, 'temple_id': temple_id, 'priority': priority, 'status': status, 'owner': owner,
           'send_at': send_at, 'sent': sent, 'total': total, 'created_at': created_at if created_at else int(time.time()),
           'updated_at': updated_at if updated_at else int(time.time())}
    result = do_tidb_exe(sql=sql, args=key)
    return result
...
def insert_noti_temple(project, name, args, content, temple_desc=None):
    created_at = int(time.time())
    updated_at = int(time.time())
    sql = """insert {project}_noti_temple (name,args,temple_desc,content,created_at,updated_at ) values (%(name)s,%(args)s,%(temple_desc)s,%(content)s,%(created_at)s,%(updated_at)s)""".format(
        project=project)
...
def insert_usergroup_plan(project, group_title, group_desc, repeatable, priority, enable_policy, func_args):
    sql = """insert {project}_usergroup_plan (group_title,group_desc,func,repeatable,priority,enable_policy,created_at,updated_at) values (%(group_title)s,%(group_desc)s,%(func_args)s,%(repeatable)s,%(priority)s,%(enable_policy)s,%(created_at)s,%(updated_at)s)""".format(
        project=project)
    timenow = int(time.time())
    key = {'group_title': group_title, 'group_desc': group_desc, 'repeatable': repeatable, 'priority': priority, 'enable_policy': enable_policy,
           'func_args': func_args, 'created_at': timenow, 'updated_at': timenow}
    result = do_tidb_exe(sql=sql, args=key)
    return result
