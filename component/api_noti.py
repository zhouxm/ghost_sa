...
from component.db_func import show_project_usergroup_plan, show_project_usergroup_list, duplicate_scheduler_jobs_sql, select_usergroup_data_for_api, \
    select_usergroup_datacount_for_api, disable_usergroup_data_db, show_temples_db, show_noti_group_db, show_noti_group_count_db, show_noti_db, \
    show_noti_count_db, select_noti_group, select_noti_single, disable_noti_db, show_scheduler_jobs_db, show_scheduler_jobs_count_db, \
    select_usergroup_jobs_plan_manual, insert_scheduler_job
import json
        ...
        try:
            ...
            for item in results[0]:
                if mode and mode == 'cli':
                    temp_json.append({"plan_id": item[0], "group_title": item[1], "latest_data_list_index": item[3], "repeatable": item[4], "priority": item[6],
                                      "enable_policy": item[8], "repeat_times": item[9], "latest_data_time": item[10], "latest_apply_temple_name": item[12],
                                      "latest_apply_temple_time": item[13], "updated_at": item[15]})
                else:
                    temp_json.append(
                        {"plan_id": item[0], "group_title": item[1], "group_desc": item[2], "latest_data_list_index": item[3], "repeatable": item[4],
                         "priority_id": item[5], "priority": item[6], "enable_policy_id": item[7], "enable_policy": item[8], "repeat_times": item[9],
                         "latest_data_time": item[10], "latest_apply_temple_id": item[11], "latest_apply_temple_name": item[12],
                         "latest_apply_temple_time": item[13], "created_at": item[14], "updated_at": item[15]})
...
def show_usergroup_list():
    ...
    if password == admin.admin_password and project and request.method == 'POST' and plan_id:  # 只有正确的密码才能触发动作
        ...
        try:
            ...
            for item in results[0]:
                if mode and mode == 'cli':
                    temp_json.append({"list_id": item[0], "group_id": item[1], "group_title": item[2], "group_list_index": item[3], "list_init_date": item[4],
                                      "list_desc": item[5], "jobs_id": item[6], "item_count": item[7], "status_name": item[9], "complete_at": item[10],
                                      "apply_temple_times": item[11], "created_at": item[12], "updated_at": item[13]})
                else:
                    temp_json.append({"list_id": item[0], "group_id": item[1], "group_title": item[2], "group_list_index": item[3], "list_init_date": item[4],
                                      "list_desc": item[5], "jobs_id": item[6], "item_count": item[7], "status_id": item[8], "status_name": item[9],
                                      "complete_at": item[10], "apply_temple_times": item[11], "created_at": item[12], "updated_at": item[13]})
...
def show_usergroup_data():
    ...
    if password == admin.admin_password and project and request.method == 'POST' and list_id:  # 只有正确的密码才能触发动作
        ...
        try:
            ...
            for item in results[0]:
                temp_json.append(
                    {"group_id": item[0], "list_id": item[1], "data_id": item[2], "data_index": item[3], "data_key": item[4], "data_json": json.loads(item[5]),
                     "enable_policy_id": item[6], "enable_policy_name": item[7]})
            time_cost = round(time.time() - start_time, 2)
            total_count = resultscount[0][0][0] if resultscount[1] > 0 else 0
            returnjson = {'result': 'success', 'results_count': results[1], 'timecost': time_cost, 'data': temp_json, 'total_count': total_count, 'page': page,
                          'length': length}
            return jsonify(returnjson)
        except Exception:
            error = traceback.format_exc()
    ...
    if password == admin.admin_password and project and request.method == 'POST':  # 只有正确的密码才能触发动作
        ...
        try:
            ...
            for item in results[0]:
                if mode and mode == 'cli':
                    temp_json.append(
                        {"temple_id": item[0], "temple_name": item[1], "temple_desc": item[2], "apply_times": item[5], "lastest_apply_time": item[6],
                         "lastest_apply_list_desc": item[8], "lastest_apply_group_name": item[9], "created_at": item[10], "updated_at": item[11]})
                else:
                    temp_json.append({"temple_id": item[0], "temple_name": item[1], "temple_desc": item[2], "temple_args": json.loads(item[3]),
                                      "temple_content": json.loads(item[4]), "apply_times": item[5], "lastest_apply_time": item[6],
                                      "lastest_apply_list_id": item[7], "lastest_apply_list_desc": item[8], "lastest_apply_group_name": item[9],
                                      "created_at": item[10], "updated_at": item[11]})
...
def apply_temples_list():
    ...
    if password == admin.admin_password and project and owner and request.method == 'POST':  # 只有正确的密码才能触发动作
        try:
            results = create_noti_group(project=project, temple_id=temple_id, user_group_id=user_group_id, data_id=data_id, owner=owner,
                                        send_at=send_at)  # 实际开始应用模板（单条数据和分群列表同时存在时，使用单条数据）
            temp_json = []
            time_cost = round(time.time() - start_time, 2)
...
def show_noti_group():
    ...
    if password == admin.admin_password and project and request.method == 'POST':
        # 只有正确的密码才能触发动作
        add_on_where = ''
        add_on_where = add_on_where + (
            ''' and {project}_noti_group.plan_id = {plan_id}'''.format(project=project, plan_id=int(plan_id)) if plan_id and plan_id != '' else '')
        add_on_where = add_on_where + (
            ''' and {project}_noti_group.list_id = {list_id}'''.format(project=project, list_id=int(list_id)) if list_id and list_id != '' else '')
        add_on_where = add_on_where + (
            ''' and {project}_noti_group.temple_id = {temple_id}'''.format(project=project, temple_id=int(temple_id)) if temple_id and temple_id != '' else '')
        ...
        try:
            ...
            for item in results[0]:
                if mode and mode == 'cli':
                    temp_json.append({"noti_group_id": item[0], "plan_title": item[2], "list_id": item[3], "list_init_date": item[4], "list_desc": item[5],
                                      "jobs_id": item[6], "item_count": item[7], "list_status": item[9], "temple_name": item[12], "priority_name": item[14],
                                      "status_name": item[16], "owner": item[17], "send_at": item[18], "created_at": item[19], "updated_at": item[20],
                                      "sent_success": str(item[21]) + "/" + str(item[22])})
                else:
                    temp_json.append({"noti_group_id": item[0], "plan_id": item[1], "plan_title": item[2], "list_id": item[3], "list_init_date": item[4],
                                      "list_desc": item[5], "jobs_id": item[6], "item_count": item[7], "list_status_id": item[8], "list_status": item[9],
                                      "data_id": item[10], "temple_id": item[11], "temple_name": item[12], "priority_id": item[13], "priority_name": item[14],
                                      "status_id": item[15], "status_name": item[16], "owner": item[17], "send_at": item[18], "created_at": item[19],
                                      "updated_at": item[20], "sent": item[21], "total": item[22]})
            time_cost = round(time.time() - start_time, 2)
            total_count = resultscount[0][0][0] if resultscount[1] > 0 else 0
            returnjson = {'result': 'success', 'results_count': results[1], 'timecost': time_cost, 'data': temp_json, 'total_count': total_count, 'page': page,
                          'length': length}
...
def show_noti_detial():
    ...
    if password == admin.admin_password and project and request.method == 'POST' and noti_group_id:
        # 只有正确的密码才能触发动作
        add_on_where = ''
        add_on_where = add_on_where + (''' and {project}_noti.noti_group_id = {noti_group_id}'''.format(project=project, noti_group_id=int(
            noti_group_id)) if noti_group_id and noti_group_id != '' else '')
        add_on_where = add_on_where + (
            ''' and {project}_noti.plan_id = {plan_id}'''.format(project=project, plan_id=int(plan_id)) if plan_id and plan_id != '' else '')
        add_on_where = add_on_where + (
            ''' and {project}_noti.list_id = {list_id}'''.format(project=project, list_id=int(list_id)) if list_id and list_id != '' else '')
        add_on_where = add_on_where + (
            ''' and {project}_noti.temple_id = {temple_id}'''.format(project=project, temple_id=int(temple_id)) if temple_id and temple_id != '' else '')
        add_on_where = add_on_where + (
            ''' and {project}_noti.id = {noti_id}'''.format(project=project, noti_id=int(noti_id)) if noti_id and noti_id != '' else '')
        if not mode or mode != 'cli':
        ...
        try:
            ...
            for item in results[0]:
                if mode and mode == 'cli':
                    temp_json.append(
                        {"noti_id": item[0], "plan_name": item[2], "list_desc": item[5], "data_id": item[8], "temple_name": item[10], "noti_group_id": item[11],
                         "distinct_id": item[12], "type_name": item[14], "priority_name": item[17], "status_name": item[19], "owner": item[20],
                         "recall_result": item[21], "send_at": item[22], "created_at": item[23], "updated_at": item[24]})
                else:
                    temp_json.append(
                        {"noti_id": item[0], "plan_id": item[1], "plan_name": item[2], "list_id": item[3], "list_init_date": item[4], "list_desc": item[5],
                         "jobs_id": item[6], "list_status": item[7], "data_id": item[8], "temple_id": item[9], "temple_name": item[10],
                         "noti_group_id": item[11], "distinct_id": item[12], "type_id": item[13], "type_name": item[14], "content": json.loads(item[15]),
                         "priority_id": item[16], "priority_name": item[17], "status_id": item[18], "status_name": item[19], "owner": item[20],
                         "recall_result": item[21], "send_at": item[22], "created_at": item[23], "updated_at": item[24]})
            time_cost = round(time.time() - start_time, 2)
            total_count = resultscount[0][0][0] if resultscount[1] > 0 else 0
            returnjson = {'result': 'success', 'results_count': results[1], 'timecost': time_cost, 'data': temp_json, 'total_count': total_count, 'page': page,
    ...
    if password == admin.admin_password and request.method == 'POST':
        try:
            ...
            for item in results[0]:
                if mode and mode == 'cli':
                    temp_json.append(
                        {"scheduler_id": item[0], "project": item[1], "group_id": item[2], "list_index": item[3], "start_at": item[4], "priority_name": item[7],
                         "status_name": item[9], "created_at": item[10], "updated_at": item[11]})
                else:
                    temp_json.append({"scheduler_id": item[0], "project": item[1], "group_id": item[2], "list_index": item[3], "start_at": item[4],
                                      "data": json.loads(item[5]), "priority_id": item[6], "priority_name": item[7], "status_id": item[8],
                                      "status_name": item[9], "created_at": item[10], "updated_at": item[11]})
            returnjson = {'result': 'success', 'results_count': results[1], 'timecost': time_cost, 'data': temp_json, 'total_count': result_count[0][0][0],
    ...
    if password == admin.admin_password and request.method == 'POST':
        try:
            ...
            for plan in plan_result:
                ...
                for time_1 in times:
                    insert_result, insert_count = insert_scheduler_job(project=project, group_id=plan[0], datetime=time_1['time_int'],
                                                                       data={'datetime_int': time_1['time_int'], 'datetime_tuple': time_1['time_tuple'],
                                                                             'datetime': time.strftime("%Y-%m-%d %H:%M:%S", time_1['time_tuple']),
                                                                             'date': time.strftime("%Y-%m-%d", time_1['time_tuple']),
                                                                             'func': json.loads(plan[1])}, priority=plan[3] if plan[3] else 13, status=16)
                    write_to_log(filename='api_noti', defname='create_scheduler_jobs_manual',
                                 result='项目' + str(project) + '计划' + str(plan[0]) + '已添加时间' + time.strftime("%Y-%m-%d %H:%M:%S", time_1['time_tuple']))
