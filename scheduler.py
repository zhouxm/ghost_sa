...
from component.db_func import select_scheduler_enable_project, select_usergroup_jobs_plan, insert_scheduler_job, check_next_scheduler_job, update_scheduler_job, \
    select_noti_temple, select_usergroup_data, insert_noti, insert_noti_group, update_noti_group, select_usergroupdata_data, update_usergroup_plan, \
    ...
    for project in projects_result:
        ...
        for plan in plan_result:
            ...
            for time_1 in times:
                ...
                insert_scheduler_job(project=project[0], group_id=plan[0], datetime=time_1['time_int'],
                                     data={'datetime_int': time_1['time_int'], 'datetime_tuple': time_1['time_tuple'],
                                           'datetime': time.strftime("%Y-%m-%d %H:%M:%S", time_1['time_tuple']),
                                           'date': time.strftime("%Y-%m-%d", time_1['time_tuple']), 'func': func_loads}, priority=plan[3] if plan[3] else 13,
                                     status=16)
                write_to_log(filename='scheduler', defname='get_task_day',
                             result='项目' + str(project[0]) + '计划' + str(plan[0]) + '已添加时间' + time.strftime("%Y-%m-%d %H:%M:%S", time_1['time_tuple']))
...
def do_all_task():
    ...
    while task_count < 1 and int(time.time()) - start_time <= 1800:
        ...
        if task_count == 0 and priority == 13:
            ...
            try:
                write_to_log(filename='scheduler', defname='do_all_task',
                             result='优先级' + str(priority) + '任务id' + str(task_result[0][0]) + '拼接的任务参数' + str(data) + '开始执行')
                func_result, list_index = ff(data['func']['args'])
                update_scheduler_job(jobid=task_result[0][0], list_index=list_index, status=19)
                write_to_log(filename='scheduler', defname='do_all_task',
                             result='优先级' + str(priority) + '任务id' + str(task_result[0][0]) + '拼接的任务参数' + str(data) + '执行完毕')
            except Exception:
...
def create_noti_group(project, temple_id, user_group_id=None, data_id=None, owner='noti', send_at=None):
    ...
    if not user_group_id and not data_id:
        ...
        result_group = insert_noti_group(project=project, plan_id=result_data[0][0][0], list_id=result_data[0][0][1], data_id=result_data[0][0][2],
                                         temple_id=result_temple[0][0][0], owner=owner, send_at=send_at if send_at else timenow, sent=0, total=result_data[1],
                                         priority=13, status=result_data[0][0][6])
        for noti in result_data[0]:
            timenow = int(time.time())
            temple_content = apply_temple(project=project, temple_args=json.loads(result_temple[0][0][2]), temple_content=json.loads(result_temple[0][0][3]),
                                          data_json=json.loads(noti[5]), data_key=noti[4], group_id=user_group_id, owner=owner,
                                          send_at=send_at if send_at else timenow)
            insert_result = insert_noti(project=project, type_1=json.loads(result_temple[0][0][2])['meta']['medium_id'], created_at=timenow, updated_at=timenow,
                                        distinct_id=noti[4], content=temple_content, send_at=send_at if send_at else timenow, plan_id=noti[0], list_id=noti[1],
                                        data_id=noti[2], temple_id=result_temple[0][0][0], noti_group_id=result_group[2], priority=13, status=noti[6],
                                        owner=owner, recall_result=None)
        update_noti_temple(project=project, temple_id=temple_id, apply_times=1, lastest_apply_time=timenow,
            ...
            for tid in id_list:
                ...
                if result_temple[1] > 0 and result_temple[0][0][2]:
                    ...
                    for send_int in send_at:
                        create_noti_group(project=project[0], temple_id=tid, user_group_id=data_list[1], data_id=None, owner='create_auto_group',
                                          send_at=send_int)
