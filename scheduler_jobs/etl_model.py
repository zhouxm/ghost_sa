...
def apply_temple(project, temple_args, temple_content, data_json, data_key, send_at=None, group_id=None, owner='system'):
    ...
    func_result = None
    read_tracker = {"distinct_id": data_key, "event": "recall", "lib": {"$lib": "noti"}, "project": project,
                    "properties": {"$latest_utm_campaign": "___utm_campaign___", "$latest_utm_content": "___utm_content___",
                                   "$latest_utm_medium": "___utm_medium___", "$latest_utm_source": "___utm_source___", "$latest_utm_term": "___utm_term___",
                                   "$lib": "noti", "_latest_utm_email": "___email___", "_latest_utm_mobile": "___mobile___", "action": 2,
                                   "sent_time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(send_at))}, "type": "track"}
    ...
    for l in temple_content:
        for t in read_tracker['properties']:
            ...
            for d in data_json:
                temple_content[l] = temple_content[l].replace('___' + d + '___', str(data_json[d])).replace('___etl_date___',
                                                                                                            time.strftime("%Y-%m-%d", time.localtime(send_at)))
                read_tracker['properties'][t] = str(read_tracker['properties'][t]).replace('___' + d + '___', str(data_json[d])).replace('___etl_date___',
                                                                                                                                         time.strftime(
                                                                                                                                             "%Y-%m-%d",
                                                                                                                                             time.localtime(
                                                                                                                                                 send_at)))
    ...
    track_url = temple_args['ghost_sa']['track_url'] + '?project=' + project + '&data=' + urllib.parse.quote(
        base64.b64encode((gzip.compress(json.dumps(read_tracker).encode('utf-8'))))) + '&gzip=1' + (
                    '&remark=' + temple_args['ghost_sa']['remark'] if 'remark' in temple_args['ghost_sa'] else '')
...
def insert_usergroup(project, group_id, data, list_desc, jobs_id, init_time=int(time.time())):
    ...
    try:
        ...
        for item in data['data_list']:
            ...
            insert_usergroup_data(project=project, group_list_id=list_id[0][0], data_index=data_index, key=item['key'], json=json.dumps(item['json']),
                                  enable=item['enable'])
            insert_usergroup_list(project=project, group_id=group_id, group_index=index_id + 1, list_init_date=init_time, status=3, complete_at=0,
                                  apply_temple_times=0, item_add=1, created_at=None, updated_at=None, jobs_id=jobs_id)
        ...
        update_usergroup_plan(project=project, plan_id=group_id, latest_data_list_index=index_id + 1, updated_at=int(time.time()), repeat_times_add=1,
                              latest_data_time=int(time.time()))
        return 5, index_id + 1
    except Exception:
...
if __name__ == "__main__":
    print(apply_temple(project='tvcbook', temple_args={"add_on_func": {"dir": "scheduler_jobs.tvcbook", "name": "last_1500_email", "required": False},
                                                       "args": {"content": "___content___", "email": "___email___", "nickname": "___nickname___",
                                                                "subject": "___subject___", "utm_campaign": "system", "utm_content": "___group_id___",
                                                                "utm_email": "___email___", "utm_medium": "noti_temple_1", "utm_source": "邮件",
                                                                "utm_term": "___etl_date___"}, "ghost_sa": {"track_url": "https://t.tvcbook.com/sa.gif"},
                                                       "meta": {"medium": "email"}}, temple_content={
        'content': '这是一个测试内容，用户昵称是___nickname___，跳转链接是http://www.tvcbook.com/?utm_source=___utm_source___&utm_campaign=___utm_campaign___&utm_term=___utm_term___&utm_content=___utm_content___&utm_medium=___utm_medium___,跟踪链接是___read_tracker___，',
        'subject': '___subject___'},
                       data_json={"email": "lixing523@hotmail.com", "last_active_time": "2020-09-23 23:39:36", "nickname": "李星", "group_id": 1299},
                       data_key='8bd8ce59dbf486e0'))
